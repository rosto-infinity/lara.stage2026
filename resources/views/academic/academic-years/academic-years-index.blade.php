...
@section('content')
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        <x-stat-card label="Total années" value="{{ \App\Models\Academic\AcademicYear::count() }}" color="gray">
            <x-slot name="icon">
        ...
        <x-stat-card label="Année active" value="{{ \App\Models\Academic\AcademicYear::where('est_active', true)->count() }}" color="green">
            <x-slot name="icon">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <x-stat-card label="Années inactives" value="{{ \App\Models\Academic\AcademicYear::where('est_active', false)->count() }}" color="gray">
            <x-slot name="icon">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            @if($year->est_active)
                                <form action="{{ route('academic.academic-years.deactivate', $year->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
